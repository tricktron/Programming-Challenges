...
int main() {
    ...
    while (scanf("%d %d", &M, &N) == 2) {
        ...
        FORN(i, 0, N) {
            ...
            tables[capacity].emplace_back(Table(i, capacity));
        }
        ...
        FORN(i, 0, M) {
            ...
            FORN(j, 0, team.size) {
                int k = max_capacity;
                while (k > 0 && tables[k].empty()) k--;
                max_capacity = k;
                Table table = tables[k].back();
                tables[k].pop_back();
            ...
            for (const auto &table: temp) {
                if (table.capacity > max_capacity) {
                    max_capacity = table.capacity;
                }
                tables[table.capacity].push_back(table);
            }
        }
