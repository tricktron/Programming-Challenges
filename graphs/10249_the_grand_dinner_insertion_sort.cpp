...
int main() {
    ...
    while (scanf("%d %d", &M, &N) == 2) {
        ...
        FORN(i, 0, N) {
            int capacity;
            scanf("%d", &capacity);
            tables.emplace_back(Table(i, capacity));
        }

        bool validSeating = true;

        // insertion sort on teams
        FORN(j, 1, M) {
            for (int k = j; k > 0 && teams[k].size > teams[k - 1].size; k--) {
                swap(teams[k], teams[k - 1]);
            }
        }
        ...
        FORN(i, 0, M) {
            ...
            // insertion sort on tables
            FORN(j, 1, N) {
                for (int k = j; k > 0 && tables[k].capacity > tables[k - 1].capacity; k--) {
                    swap(tables[k], tables[k - 1]);
                }
            }

            // use of stack to keep partial sorting order and speed up insertion sort
            stack<Table> stack;
            FORN(j, 0, team.size) {
